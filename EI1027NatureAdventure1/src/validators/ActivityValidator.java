...
public class ActivityValidator implements Validator {
	...
	public void validate(Object obj, Errors errors) {
		...
		if(activity.getMinimumGroup()<=0){
			if(language.equals("ES")){
				errors.rejectValue("minimumGroup","cantidad","El minimo de personas es mayor que 0");//El minimo de personas es mayor que 0
			}else if(language.equals("EN") || language != null){
		...
		if(activity.getMaximumGroup()<= activity.getMinimumGroup()){
			if(language.equals("ES")){
				errors.rejectValue("maximumGroup","diferencia", "Debe ser mayor que el grupo minimo");//Debe ser mayor que el grupo minimo
			}else if(language.equals("EN") || language != null){
